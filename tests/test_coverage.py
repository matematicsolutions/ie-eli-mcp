...
from ie_eli_mcp.coverage import build_coverage
from ie_eli_mcp.server import ie_coverage


def _coverage():
    fn = getattr(ie_coverage, "fn", ie_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("IE_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("ie_coverage" in line for line in lines), (
        "an audit entry was written but it does not name ie_coverage"
    )
