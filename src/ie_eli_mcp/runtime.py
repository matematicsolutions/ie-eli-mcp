...
Pulls a tiny static ``ie-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/ie-runtime.json
...
Opt-out: set ``IE_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "IE_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/ie-eli-mcp/releases/latest/download/ie-runtime.json.gz",
)
_CACHE_NAME = "ie-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "ie-eli-mcp-runtime (+https://github.com/matematicsolutions/ie-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("IE_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "ie-eli"
